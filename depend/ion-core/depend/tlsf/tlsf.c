...
{
	return ion_FindFirstSetBit32(word);
}
...
static inline int tlsf_fls_sizet(size_t size)
{
	...
	const int ulong_bits = 8 * sizeof(size_t);
	const int bit = size ? ulong_bits - ion_CountLeadingZeroes64(size) : 0;
#else
	const int ulong_bits = 8 * sizeof(unsigned long);
	const int bit = size ? ulong_bits - ion_CountLeadingZeroes32((unsigned long)size) : 0;
#endif
	return bit - 1;
}
...
static block_header_t *block_locate_free(tlsf_t *tlsf, size_t size)
{
	...
	if (size > 0) {
		...
		if (size >= SMALL_BLOCK_SIZE) 
		{
			const size_t round = (1ull << (tlsf_fls_sizet(size) - SL_INDEX_COUNT_LOG2)) - 1;
			adjusted += round;
		}
...
size_t tlsf_smallblock_size(void)
{
...
tlsf_pool_t *tlsf_add_pool(tlsf_t *tlsf, void *mem, size_t bytes)
{
	...
	assert(pool_bytes >= block_size_min && pool_bytes < block_size_max);
#if 0
...
int test_ffs_fls()
{
	...
	rv += (tlsf_fls_sizet(0) == -1) ? 0 : 0x2;
	rv += (tlsf_ffs(1) == 0) ? 0 : 0x4;
	rv += (tlsf_fls_sizet(1) == 0) ? 0 : 0x8;
	rv += (tlsf_ffs(0x80000000) == 31) ? 0 : 0x10;
	rv += (tlsf_ffs(0x80008000) == 15) ? 0 : 0x20;
	rv += (tlsf_fls_sizet(0x80000008) == 31) ? 0 : 0x40;
	rv += (tlsf_fls_sizet(0x7FFFFFFF) == 30) ? 0 : 0x80;
